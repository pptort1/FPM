...
export default function SubirVentasBsale({ onSubido }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const res = await api.post<Resultado>("/ingresos/importar-bsale", form, {
  ...
  return (
    <>
      <button onClick={() => { setOpen(true); setResultado(null); setError(null); }}
        ...
        <Upload size={15} /> Subir ventas Bsale
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                <h2 className="font-semibold text-gray-800">Importar ventas Bsale</h2>
                <p className="text-xs text-gray-400 mt-0.5">Reporte Excel de ventas exportado desde Bsale</p>
              </div>
              ...
              {!resultado && (
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                    loading ? "border-emerald-300 bg-emerald-50" : "border-gray-300 hover:border-emerald-400 hover:bg-emerald-50/50"
                  }`}>
                  ...
                  {loading ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-emerald-600 font-medium">Procesando ventas…</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <Upload size={32} className="mx-auto text-gray-400"/>
                      <p className="text-sm text-gray-600 font-medium">Arrastrá el Excel o click para seleccionar</p>
                      <p className="text-xs text-gray-400">Reporte "VentasBsale" exportado desde Bsale</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {resultado && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { label:"Documentos nuevos", val:resultado.agregados,  color:"bg-emerald-50 text-emerald-800" },
                      { label:"Ya importados",      val:resultado.duplicados, color:"bg-gray-50 text-gray-600" },
