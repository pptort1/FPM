...
export type SortKeyI = "fecha"|"mes_devengo"|"monto_total"|"monto_neto"|"canal"|"tipo_doc"|"cliente";
...
interface Props {
  ...
  sortKey: SortKeyI;
  sortDir: "asc"|"desc";
  onSort: (k: SortKeyI) => void;
}

export default function IngresosTable({ items, total, pagina, porPagina, onPagina, loading, sortKey, sortDir, onSort }: Props) {
  const totalPaginas = Math.ceil(total / porPagina);
  const desde = (pagina-1)*porPagina+1;
  const hasta = Math.min(pagina*porPagina, total);

  const Th = ({ label, sk, left }: { label: string; sk: SortKeyI; left?: boolean }) => (
          ...
          <tbody className={loading?"opacity-50":""}>
            ...
            {items.map((tx,i)=>(
              <tr key={tx.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i%2!==0?"bg-gray-50/30":""}`}>
                <td className="px-4 py-2.5 text-gray-600 whitespace-nowrap text-xs">{tx.fecha}</td>
                <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap text-xs">{tx.mes_devengo}</td>
                <td className="px-4 py-2.5 max-w-xs">
                  <div className="truncate text-xs text-gray-500" title={tx.cliente??""}>
                    {tx.cliente??"—"}
                  </div>
                </td>
                <td className="px-4 py-2.5 max-w-xs">
                ...
                <td className="px-4 py-2.5 text-center">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${CANAL_BADGE[tx.canal??""??""]??"bg-gray-100 text-gray-500"}`}>
                    {CANAL_NOMBRE[tx.canal??""??""??tx.canal??""]} {tx.canal}
                  </span>
                </td>
